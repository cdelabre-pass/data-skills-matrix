<script lang="ts">
	...
	function selectAll() {
		selected = new Set(categories.map(c => c.id).filter(id => getRelevantSkillCount(id) > 0));
	}
	...
	$: totalSkills = Array.from(selected).reduce((sum, catId) => sum + getRelevantSkillCount(catId), 0);
	$: estimatedMinutes = Math.ceil(totalSkills * 0.5);
</script>

<div class="space-y-8">
	<div class="text-center">
		<h2 class="text-2xl md:text-3xl font-bold text-base-100 mb-3">Domaines à évaluer</h2>
		<p class="text-base-400">Sélectionnez au moins {MIN_CATEGORIES} catégories pour obtenir un radar complet et une évaluation pertinente.</p>
	</div>
	...
	<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
		{#each categories as category, i}
			...
			>
				...
				{#if isSelected}
					<div class="absolute inset-0 bg-gradient-to-br {categoryColors[category.id] || 'from-accent-500/20 to-accent-600/20'} opacity-50"></div>
				{/if}
	...
	<!-- Summary and confirm -->
	{#if selected.size > 0}
		<div class="card p-6 {canContinue ? 'border-accent-500/20 bg-accent-500/5' : 'border-amber-500/20 bg-amber-500/5'} animate-fade-in-up">
			<div class="flex flex-col sm:flex-row items-center justify-between gap-4">
				<div class="flex items-center gap-4">
					<div class="w-12 h-12 rounded-xl {canContinue ? 'bg-accent-500/20' : 'bg-amber-500/20'} flex items-center justify-center">
						<span class="font-mono font-bold text-xl {canContinue ? 'text-accent-400' : 'text-amber-400'}">{totalSkills}</span>
					</div>
					<div>
						<p class="font-medium text-base-100">compétences sélectionnées</p>
						{#if canContinue}
							<p class="text-sm text-base-500">Durée estimée : ~{estimatedMinutes} min</p>
						{:else}
							<p class="text-sm text-amber-400">
								Encore {remainingToSelect} domaine{remainingToSelect > 1 ? 's' : ''} à sélectionner pour le radar
							</p>
						{/if}
					</div>
				</div>
				<button
					...
					class="btn btn-lg w-full sm:w-auto {canContinue ? 'btn-primary shadow-glow' : 'btn-ghost opacity-50 cursor-not-allowed'}"
				>
					Continuer
