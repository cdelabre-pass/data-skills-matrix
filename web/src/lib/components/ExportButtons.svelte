<script lang="ts">
	...
	function getExpectedLevel(
		skill: any,
		roleId: string,
		levelIndex: number = suggestedLevelIndex,
	): number | string {
	...
	function calculateGap(
		currentLevel: number | 'nc' | undefined,
		expectedLevel: number | string,
	): string {
	...
	function getTopGaps(
		...
		return allSkills
			.map((skill) => {
				...
				if (expectedLevel === 'NC' || typeof expectedLevel !== 'number')
					return null;
				const gap = expectedLevel - currentLevel;
	...
	function createDescriptionsSheet(wb: ExcelJS.Workbook) {
		...
		for (const category of skillsData.categories) {
			...
			for (const skill of categorySkills) {
				...
				// Level descriptions 0-6 (including bonus levels 5-6)
				...
				// Resources — real hyperlink for single resource, readable text for multiple
				const resourceCell = row.getCell(10);
				if (skill.resources) {
					if (Array.isArray(skill.resources)) {
						if (skill.resources.length === 1) {
							const r = skill.resources[0];
							resourceCell.value = { text: r.title || r.url, hyperlink: r.url };
							resourceCell.font = {
								color: { argb: 'FF0563C1' },
								underline: true,
							};
						} else {
							resourceCell.value = skill.resources
								.map((r: any) => `• ${r.title}\n  ${r.url}`)
								.join('\n\n');
						}
					} else {
				...
				resourceCell.alignment = { wrapText: true, vertical: 'top' };
				resourceCell.border = BORDERS_ALL;
		...
		ws.getColumn(1).width = 25;
		ws.getColumn(2).width = 30;
		for (let i = 3; i <= 9; i++) ws.getColumn(i).width = 40;
	...
	function createAssessmentSheet(wb: ExcelJS.Workbook) {
		...
		const role = skillsData.roles.find(
			(r: any) => r.id === assessmentState.role,
		);
		...
		if (isTemplate) {
			ws.getCell('A2').value =
				'⚠️  MODÈLE VIERGE — Remplissez la colonne "Mon Niveau" (E) pour chaque compétence. Ou complétez l\'évaluation sur le site, puis re-exportez.';
			ws.getCell('A2').fill = {
				type: 'pattern',
				pattern: 'solid',
				fgColor: { argb: 'FFFEF08A' },
			};
			ws.getCell('A2').font = { bold: true, color: { argb: 'FF92400E' } };
		} else {
			ws.getCell('A2').value =
				`Instructions: Évaluez votre niveau et fournissez des preuves concrètes (liens projets, PRs, dashboards) — Exporté le ${exportDate}`;
		...
		for (const category of skillsData.categories) {
			...
			for (const skill of categorySkills) {
				...
				row.getCell(3).value = {
					formula: `IFERROR(INDEX(Ref!B$5:${refLastCol}$${lastRefRow},MATCH(B${rowIdx},Ref!A$5:A$${lastRefRow},0),(MATCH(B$3,Ref!B$1:G$1,0)-1)*5+1),"S")`,
				};
				...
				row.getCell(4).value = {
					formula: `IFERROR(INDEX(Ref!B$5:${refLastCol}$${lastRefRow},MATCH(B${rowIdx},Ref!A$5:A$${lastRefRow},0),(MATCH(B$3,Ref!B$1:G$1,0)-1)*5+1+MATCH(D$3,Ref!B$2:E$2,0)),"NC")`,
				};
				...
				row.getCell(6).value = {
					formula: `IF(OR(E${rowIdx}="",D${rowIdx}="NC",E${rowIdx}="NC"),"",E${rowIdx}-D${rowIdx})`,
				};
		...
		// Conditional formatting on Écart column (F) — color by gap value
		if (skillRows.length > 0) {
			const firstDataRow = skillRows[0];
			const lastDataRow = skillRows[skillRows.length - 1];
	...
	function createProfilsParRoleSheet(wb: ExcelJS.Workbook) {
		...
		const levelName =
			skillsData.levels[suggestedLevelIndex]?.name || 'Confirmé';
		const monNiveauCol = 2 + skillsData.roles.length + 1; // Mon Niveau
		const monRoleExpCol = monNiveauCol + 1; // Niveau Attendu (Mon Rôle)
		const monRoleEcartCol = monNiveauCol + 2; // Écart (Mon Rôle)
		const totalCols = monRoleEcartCol;
		const lastRefRow = 4 + skillsData.skills.length; // Ref sheet data starts at row 5
		...
		ws.getCell('A3').value =
			'Changez le niveau de carrière et le rôle ci-dessous pour comparer les profils. Mon Niveau vient de la feuille Mon Évaluation. Cellules vertes = compétence core pour ce rôle.';
		ws.getCell('A3').font = {
			italic: true,
			size: 10,
			color: { argb: 'FF888888' },
		...
		// Row 4: interactive controls
		const levelNames = skillsData.levels.map((l: any) => l.name).join(',');
		const roleNames = skillsData.roles.map((r: any) => r.name).join(',');
		const userRoleName = assessmentState.role
			? skillsData.roles.find((r: any) => r.id === assessmentState.role)
					?.name || ''
			: '';

		ws.getRow(4).height = 26;

		ws.getCell('A4').value = 'Niveau de carrière :';
		ws.getCell('A4').font = { bold: true, size: 11 };
		ws.getCell('A4').alignment = { horizontal: 'right', vertical: 'middle' };

		ws.getCell('B4').value = levelName;
		ws.getCell('B4').font = {
			bold: true,
			size: 11,
			color: { argb: 'FF1A5276' },
		...
		headers.push('Mon Niveau', 'Niveau Attendu (Mon Rôle)', 'Écart');
		...
		for (const category of skillsData.categories) {
			...
			for (const skill of categorySkills) {
				...
				// Expected level per role — INDEX/MATCH formula driven by career level dropdown at B4
				let col = 3;
				for (const role of skillsData.roles) {
					const isCore = skill.core_roles?.includes(role.id);
					const initialExpected = getExpectedLevel(
						skill,
						role.id,
						suggestedLevelIndex,
					);
					const cell = row.getCell(col);
					cell.value = {
						formula: `IFERROR(INDEX(Ref!$B$5:$AE$${lastRefRow},MATCH($B${rowIdx},Ref!$A$5:$A$${lastRefRow},0),(MATCH("${escapeFormulaString(role.name)}",Ref!$B$1:$G$1,0)-1)*5+1+MATCH($B$4,Ref!$B$2:$E$2,0)),"NC")`,
						result: initialExpected === 'NC' ? 'NC' : initialExpected,
					};
					cell.alignment = { horizontal: 'center' };
					cell.border = BORDERS_ALL;
					if (isCore) {
				...
				// Niveau Attendu (Mon Rôle) — INDEX/MATCH using role selector E4 + level selector B4
				const monNiveauLetter = colToLetter(monNiveauCol);
				const monRoleExpCell = row.getCell(monRoleExpCol);
				const initialMonRoleExp = getExpectedLevel(
					skill,
					assessmentState.role || '',
					suggestedLevelIndex,
				);
				monRoleExpCell.value = {
					formula: `IFERROR(INDEX(Ref!$B$5:$AE$${lastRefRow},MATCH($B${rowIdx},Ref!$A$5:$A$${lastRefRow},0),(MATCH($E$4,Ref!$B$1:$G$1,0)-1)*5+1+MATCH($B$4,Ref!$B$2:$E$2,0)),"NC")`,
					result: initialMonRoleExp === 'NC' ? 'NC' : initialMonRoleExp,
				};
				monRoleExpCell.alignment = { horizontal: 'center' };
				monRoleExpCell.border = BORDERS_ALL;
				monRoleExpCell.fill = {
					type: 'pattern',
					pattern: 'solid',
				...
				const ecartCell = row.getCell(monRoleEcartCol);
				ecartCell.value = {
					formula: `IF(OR(${monNiveauLetter}${rowIdx}="",${monRoleExpLetter}${rowIdx}="NC"),"",${monNiveauLetter}${rowIdx}-${monRoleExpLetter}${rowIdx})`,
				};
				ecartCell.alignment = { horizontal: 'center' };
				ecartCell.border = BORDERS_ALL;
		...
		skillsData.categories.forEach((cat: any, i: number) => {
			...
			ws.getCell(r, 3).value = {
				formula: `AVERAGEIF('Mon Évaluation'!$A$6:$A$${evalLastRow},A${r},'Mon Évaluation'!$E$6:$E$${evalLastRow})`,
			};
		...
		ws.getColumn(1).width = 25;
		ws.getColumn(2).width = 35;
		for (let i = 3; i <= monNiveauCol; i++) {
			ws.getColumn(i).width = 15;
		}
		ws.getColumn(monRoleExpCol).width = 22;
		ws.getColumn(monRoleEcartCol).width = 10;
	...
	async function injectNativeRadarChart(
		...
		const sheetMatch = workbookXml.match(
			new RegExp(
				`name="${sheetName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"[^>]*r:id="(rId\\d+)"`,
			),
		);
		...
		const workbookRels = await zip
			...
			.async('string');
		const targetMatch = workbookRels.match(
...
<c:valAx><c:axId val="222222222"/><c:scaling><c:orientation val="minMax"/><c:max val="6"/><c:min val="0"/></c:scaling><c:delete val="0"/><c:axPos val="l"/><c:crossAx val="111111111"/><c:numFmt formatCode="0" sourceLinked="0"/></c:valAx>
</c:plotArea>
...
<c:plotVisOnly val="1"/>
		...
		const chartCol = 2 + skillsData.roles.length + 4; // After Mon Niveau + Niveau Attendu (Mon Rôle) + Écart + 1 gap
...
<Relationship Id="${drawingRId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing" Target="../drawings/drawing1.xml"/>
	...
	function getExhaustiveGaps(): {
		currentLevelGaps: GapItem[];
		nextLevelGaps: GapItem[];
	} {
		...
		for (const category of skillsData.categories) {
			...
			for (const skill of categorySkills) {
				...
				if (
					!currentLevelGapSkillIds.has(skill.id) &&
					suggestedLevelIndex + 1 < levels.length
				) {
	...
	function createPlanDeveloppementSheet(wb: ExcelJS.Workbook) {
		...
		const levelName =
			skillsData.levels[suggestedLevelIndex]?.name || 'Confirmé';
		const nextLevelName =
			skillsData.levels[suggestedLevelIndex + 1]?.name || '';
		...
		ws.getColumn(11).width = 10; // Année
		ws.getColumn(12).width = 15; // Statut
	}

	function writeGapRows(
		...
		for (const item of gaps) {
			...
			row.getCell(1).fill = {
				type: 'pattern',
				pattern: 'solid',
				fgColor: { argb: PRIORITY_COLORS[item.priority] },
			};
			row.getCell(1).alignment = { horizontal: 'center' };
			...
			row.getCell(2).fill = {
				type: 'pattern',
				pattern: 'solid',
				fgColor: { argb: item.isCore ? COLORS.core : COLORS.secondary },
			};
			...
			row.getCell(7).font = {
				bold: true,
				color: { argb: item.gap >= 2 ? 'DC2626' : 'F59E0B' },
			};
			row.getCell(7).border = BORDERS_ALL;
			...
			row.getCell(11).value = currentYear;
			row.getCell(11).dataValidation = {
				type: 'list',
				allowBlank: true,
				formulae: [`"${currentYear},${currentYear + 1},${currentYear + 2}"`],
			};
			row.getCell(11).alignment = { horizontal: 'center' };
			row.getCell(11).border = BORDERS_ALL;

			// L: Statut
			row.getCell(12).value = 'Non démarré';
			row.getCell(12).dataValidation = {
				type: 'list',
				allowBlank: true,
				formulae: ['"Non démarré,En cours,Complété"'],
			};
			row.getCell(12).border = BORDERS_ALL;
	...
	function createHistoriqueSheet(wb: ExcelJS.Workbook) {
		...
		// Conditional formatting on score columns (D, G, J, M = Éval 1-4 scores)
		const lastHistRow = 3 + skillsData.skills.length;
		const cfRules: ExcelJS.ConditionalFormattingRule[] = [
			...
			style: {
				fill: {
					...
					fgColor: { argb: COLORS.levels[String(lvl)] },
				},
			},
		}));
		for (const scoreCol of ['D', 'G', 'J', 'M']) {
			ws.addConditionalFormatting({
				ref: `${scoreCol}4:${scoreCol}${lastHistRow}`,
				rules: cfRules,
			});
		}
	...
	// ============================================
	// Synthèse (Dashboard) Sheet
	// ============================================

	function createSynthèseSheet(wb: ExcelJS.Workbook) {
		const ws = wb.addWorksheet('Synthèse');
		const userName = assessmentState.name || 'Non renseigné';
		const role = skillsData.roles.find(
			(r: any) => r.id === assessmentState.role,
		);
		const roleName = role?.name || '';
		const levelName = skillsData.levels[suggestedLevelIndex]?.name || '';
		const exportDate = new Date().toLocaleDateString('fr-FR');
		const isTemplate =
			!assessmentState.answers ||
			Object.keys(assessmentState.answers).length === 0;
		...
		// Title
		ws.mergeCells('A1:F1');
		ws.getCell('A1').value = `Tableau de Bord – ${userName}`;
		ws.getCell('A1').fill = getHeaderFill();
		ws.getCell('A1').font = { bold: true, size: 14, color: { argb: 'FFFFFF' } };

		// Subtitle
		ws.mergeCells('A2:F2');
		ws.getCell('A2').value =
			`${roleName ? `Rôle : ${roleName}  |  ` : ''}Niveau : ${levelName}  |  Exporté le ${exportDate}`;
		ws.getCell('A2').font = { italic: true, size: 10 };

		if (isTemplate) {
			ws.mergeCells('A4:F4');
			ws.getCell('A4').value =
				'⚠️  Ce fichier est un modèle vierge. Complétez votre évaluation sur le site, puis re-exportez pour voir votre tableau de bord.';
			ws.getCell('A4').fill = {
				type: 'pattern',
				pattern: 'solid',
				fgColor: { argb: 'FFFEF08A' },
			};
			ws.getCell('A4').font = { bold: true, color: { argb: 'FF92400E' } };
		...
		// Compute stats
		const { currentLevelGaps } = getExhaustiveGaps();
		const allAnswered = skillsData.skills.filter((s: any) => {
			const a = assessmentState.answers[s.id];
			return a && a.level !== undefined && a.level !== 'nc';
		});
		const totalAnswered = allAnswered.length;
		const totalSkills = skillsData.skills.length;
		const masteredCount = allAnswered.filter((skill: any) => {
			const a = assessmentState.answers[skill.id];
			const expected = getExpectedLevel(skill, assessmentState.role || '');
			if (expected === 'NC' || typeof expected !== 'number') return false;
			return (a.level as number) >= expected;
		}).length;
		const avgScore =
			totalAnswered > 0
				? (
						allAnswered.reduce(
							(sum: number, s: any) =>
								sum + ((assessmentState.answers[s.id]?.level as number) || 0),
							0,
						) / totalAnswered
					).toFixed(1)
				: '-';
		...
		for (const cat of skillsData.categories) {
			const catSkills = skillsData.skills.filter(
				(s: any) => s.category === cat.id,
			);
			const levels: number[] = [];
			const expLevels: number[] = [];
			for (const skill of catSkills) {
				const a = assessmentState.answers[skill.id];
				if (!a || a.level === 'nc' || a.level === undefined) continue;
				const expected = getExpectedLevel(skill, assessmentState.role || '');
				if (expected === 'NC' || typeof expected !== 'number') continue;
				levels.push(a.level as number);
				expLevels.push(expected);
			}
			if (levels.length === 0) continue;
			categoryStats.push({
				name: cat.name,
				catId: cat.id,
				avgCurrent: levels.reduce((a, b) => a + b, 0) / levels.length,
				avgExpected: expLevels.reduce((a, b) => a + b, 0) / expLevels.length,
			});
		}
		...
		const statsHdr = ws.getCell(rowIdx, 1);
		statsHdr.value = 'RÉSUMÉ';
		statsHdr.fill = getHeaderFill();
		statsHdr.font = { bold: true, color: { argb: 'FFFFFF' } };
		rowIdx++;

		const stats: [string, string][] = [
			['Compétences évaluées', `${totalAnswered} / ${totalSkills}`],
			[
				'Maîtrisées (niveau ≥ attendu)',
				`${masteredCount} (${Math.round((masteredCount / Math.max(totalAnswered, 1)) * 100)}%)`,
			],
			['Écarts à combler', String(currentLevelGaps.length)],
			['Score moyen', String(avgScore)],
		];
		for (const [label, value] of stats) {
			ws.getCell(rowIdx, 1).value = label;
			ws.getCell(rowIdx, 1).font = { bold: true };
			ws.getCell(rowIdx, 1).border = BORDERS_ALL;
			ws.mergeCells(rowIdx, 2, rowIdx, 3);
			ws.getCell(rowIdx, 2).value = value;
			ws.getCell(rowIdx, 2).alignment = { horizontal: 'center' };
			ws.getCell(rowIdx, 2).border = BORDERS_ALL;
			rowIdx++;
		}
		rowIdx++;

		// TOP 5 PRIORITÉS section
		ws.mergeCells(rowIdx, 1, rowIdx, 6);
		const top5Hdr = ws.getCell(rowIdx, 1);
		top5Hdr.value = 'TOP 5 PRIORITÉS';
		top5Hdr.fill = {
			type: 'pattern',
			pattern: 'solid',
			fgColor: { argb: 'FFDBEAFE' },
		};
		top5Hdr.font = { bold: true, color: { argb: 'FF1E40AF' } };
		rowIdx++;

		if (currentLevelGaps.length === 0) {
			ws.mergeCells(rowIdx, 1, rowIdx, 6);
			ws.getCell(rowIdx, 1).value =
				'✅  Aucun écart — vous êtes au niveau attendu pour toutes les compétences !';
			...
			rowIdx++;
			for (const item of currentLevelGaps.slice(0, 5)) {
				ws.getCell(rowIdx, 1).value = item.priority;
				...
				ws.getCell(rowIdx, 1).border = BORDERS_ALL;
				ws.getCell(rowIdx, 2).value = item.skill.name;
				ws.getCell(rowIdx, 2).border = BORDERS_ALL;
				ws.getCell(rowIdx, 3).value = item.category.name;
				ws.getCell(rowIdx, 3).fill = getCategoryFill(item.category.id);
				ws.getCell(rowIdx, 3).border = BORDERS_ALL;
				ws.getCell(rowIdx, 4).value = item.currentLevel;
				ws.getCell(rowIdx, 4).fill = getLevelFill(item.currentLevel);
				ws.getCell(rowIdx, 4).alignment = { horizontal: 'center' };
				ws.getCell(rowIdx, 4).border = BORDERS_ALL;
				ws.getCell(rowIdx, 5).value = item.expectedLevel;
				ws.getCell(rowIdx, 5).alignment = { horizontal: 'center' };
				ws.getCell(rowIdx, 5).border = BORDERS_ALL;
				ws.getCell(rowIdx, 6).value = item.gap;
				ws.getCell(rowIdx, 6).font = {
					bold: true,
					color: { argb: item.gap >= 2 ? 'FFDC2626' : 'FFF59E0B' },
				};
				ws.getCell(rowIdx, 6).alignment = { horizontal: 'center' };
				ws.getCell(rowIdx, 6).border = BORDERS_ALL;
				rowIdx++;
			}
		}
		rowIdx++;

		// PROGRESSION PAR CATÉGORIE section
		ws.mergeCells(rowIdx, 1, rowIdx, 6);
		const catHdr = ws.getCell(rowIdx, 1);
		catHdr.value = 'PROGRESSION PAR CATÉGORIE';
		catHdr.fill = {
			type: 'pattern',
			pattern: 'solid',
			fgColor: { argb: 'FFF3E8FF' },
		};
		catHdr.font = { bold: true, color: { argb: 'FF7C3AED' } };
		rowIdx++;
		['Catégorie', 'Score Actuel', 'Score Attendu', 'Écart'].forEach((h, i) => {
			const cell = ws.getCell(rowIdx, i + 1);
			cell.value = h;
			applyHeaderStyle(cell);
		});
		...
		for (const stat of categoryStats) {
			const gapVal = Math.round((stat.avgExpected - stat.avgCurrent) * 10) / 10;
			ws.getCell(rowIdx, 1).value = stat.name;
			ws.getCell(rowIdx, 1).fill = getCategoryFill(stat.catId);
			ws.getCell(rowIdx, 1).border = BORDERS_ALL;
			ws.getCell(rowIdx, 2).value = Math.round(stat.avgCurrent * 10) / 10;
			ws.getCell(rowIdx, 2).numFmt = '0.0';
			ws.getCell(rowIdx, 2).alignment = { horizontal: 'center' };
			ws.getCell(rowIdx, 2).fill = getLevelFill(Math.round(stat.avgCurrent));
			ws.getCell(rowIdx, 2).border = BORDERS_ALL;
			ws.getCell(rowIdx, 3).value = Math.round(stat.avgExpected * 10) / 10;
			ws.getCell(rowIdx, 3).numFmt = '0.0';
			ws.getCell(rowIdx, 3).alignment = { horizontal: 'center' };
			ws.getCell(rowIdx, 3).border = BORDERS_ALL;
			ws.getCell(rowIdx, 4).value = gapVal;
			ws.getCell(rowIdx, 4).numFmt = '0.0';
			ws.getCell(rowIdx, 4).alignment = { horizontal: 'center' };
			ws.getCell(rowIdx, 4).font = {
				bold: true,
				color: {
					argb: gapVal > 1 ? 'FFDC2626' : gapVal > 0 ? 'FFF59E0B' : 'FF16A34A',
				},
			};
			ws.getCell(rowIdx, 4).border = BORDERS_ALL;
			rowIdx++;
	...
	async function exportToExcel() {
		...
		a.href = url;
		const userName = (assessmentState.name || 'export')
			.toLowerCase()
			.replace(/[^a-z0-9à-ÿ]/gi, '-')
			.replace(/-+/g, '-')
			.replace(/^-|-$/g, '');
		a.download = `${userName}-skills-matrix-${new Date().toISOString().split('T')[0]}.xlsx`;
		a.click();
	...
	async function mergeAndDownload() {
		...
		try {
			...
			const a = document.createElement('a');
			a.href = url;
			const mergeUserName = (assessmentState.name || 'export')
				.toLowerCase()
				.replace(/[^a-z0-9à-ÿ]/gi, '-')
				.replace(/-+/g, '-')
				.replace(/^-|-$/g, '');
			a.download = `${mergeUserName}-skills-matrix-${new Date().toISOString().split('T')[0]}.xlsx`;
			a.click();
			URL.revokeObjectURL(url);
...
<div class="card p-6">
	...
	<p class="text-sm text-base-500 mt-4 flex items-center gap-2">
		...
		Vos données restent sur votre appareil. Aucune information n'est envoyée à un
		serveur.
	</p>
</div>
	...
	>
		<div class="bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-xl">
			...
			<p class="text-sm text-gray-600 mb-4">
				Uploadez votre fichier Excel existant. Vos nouvelles évaluations seront
				fusionnées et l'historique sera mis à jour automatiquement.
			</p>
			...
			<div class="bg-blue-50 p-3 rounded-md mb-4">
				...
				<ul class="text-sm text-blue-700 mt-1 list-disc list-inside">
					<li>
						Mon Évaluation: nouveaux scores (commentaires manager préservés)
					</li>
					<li>
						Historique: décalage Éval 1→2→3→4, nouvelle évaluation en Éval 1
					</li>
			...
			<div class="flex justify-end gap-3">
				<button on:click={closeMergeModal} class="btn btn-ghost">
					Annuler
				</button>
				<button
