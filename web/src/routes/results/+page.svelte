<script lang="ts">
	...
	$: suggestedLevel =
		ready && skillsData && $assessmentStore.role
			? suggestCareerLevel($assessmentStore, skillsData)
			: null;
	$: suggestedLevelIdx = suggestedLevel
		? skillsData.levels.findIndex((l: any) => l.id === suggestedLevel.level)
		: 1;
	$: roleData = skillsData?.roles?.find(
		(r: any) => r.id === $assessmentStore.role,
	);
	...
	$: answeredCount = Object.values($assessmentStore.answers).filter((a: any) =>
		skillIds.has(a.skillId),
	).length;
	$: totalSkills = $assessmentStore.skills.length;
	$: unansweredCount = totalSkills - answeredCount;
	$: completionPercentage =
		totalSkills > 0 ? Math.round((answeredCount / totalSkills) * 100) : 0;
	$: showContinueCTA =
		unansweredCount > 0 || (suggestedLevel && suggestedLevel.confidence < 70);

	function continueAssessment() {
		// Find first unanswered skill
		const state = $assessmentStore;
		const firstUnansweredIndex = state.skills.findIndex(
	...
	<div class="relative max-w-6xl mx-auto px-4 py-8">
		<div class="text-center mb-8 animate-fade-in-down">
			...
			<p class="text-base-400">
				{#if isRoleAgnostic}
					Évaluation <span class="font-medium text-accent-400"
						>multi-domaines</span
					>
				{:else}
					Évaluation pour le rôle : <span class="font-medium text-accent-400"
						>{roleData?.name}</span
					>
				{/if}
			</p>
		...
		{#if showContinueCTA}
			<div
				class="mb-8 p-5 rounded-2xl bg-gradient-to-r from-accent-500/10 via-accent-500/5 to-transparent border border-accent-500/20 animate-fade-in"
			>
				<div class="flex flex-col sm:flex-row sm:items-center gap-4">
					<div class="flex-1">
						<div class="flex items-center gap-3 mb-2">
							...
							{:else}
								<div
									class="w-10 h-10 rounded-xl bg-accent-500/20 flex items-center justify-center"
								>
									<svg
						...
						<p class="text-base-400 text-sm">
							{#if unansweredCount > 0}
								Il reste <span class="text-accent-400 font-medium"
									>{unansweredCount} compétence{unansweredCount > 1
										? 's'
										: ''}</span
								> à évaluer.
							{/if}
							{#if suggestedLevel && suggestedLevel.confidence < 70}
								Votre niveau de confiance est de <span
									class="text-amber-400 font-medium"
									>{suggestedLevel.confidence}%</span
								>. Répondez à plus de questions pour obtenir une évaluation plus
								précise.
							{/if}
						</p>
						...
						<div class="mt-3 flex items-center gap-3">
							...
							<span class="text-xs text-base-500 font-mono"
		...
		<div class="mt-8 text-center">
			<a href="/" class="btn btn-secondary"> Nouvelle évaluation </a>
		</div>
	</div>
{:else}
