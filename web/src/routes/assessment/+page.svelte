<script lang="ts">
	...
	import { assessmentStore, currentSkill, currentSkillGroup, assessmentProgress, groupProgress } from '$lib/stores/assessment';
	...
	$: currentInference = $currentSkill ? $assessmentStore.inferences[$currentSkill.id] || null : null;

	// Show group transition animation when entering new group
	$: if ($currentSkillGroup?.isNewGroup && $assessmentStore.currentSkillIndex > 0) {
	...
	function handleAnswer(event: CustomEvent<{ skillId: string; level: number | 'nc' }>) {
		...
		const newlyInferred = afterInferred.filter(id => !beforeInferred.includes(id));
	...
	function goToNextStep() {
		...
		const hasInferences = Object.keys($assessmentStore.inferences).some(skillId => {
	...
	const groupIcons: Record<string, string> = {
		sql_query: 'M4 7v10c0 2 1 3 3 3h10c2 0 3-1 3-3V7c0-2-1-3-3-3H7c-2 0-3 1-3 3zm3 0h10v10H7V7zm2 2v2h2V9H9zm4 0v2h2V9h-2zm-4 4v2h6v-2H9z',
		python_ecosystem: 'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-1-4h2v2h-2v-2zm0-8h2v6h-2V8z',
		...
		ml_core: 'M12 2a10 10 0 100 20 10 10 0 000-20zm0 18a8 8 0 110-16 8 8 0 010 16zm-1-13h2v6h-2V7zm0 8h2v2h-2v-2z',
		soft_skills: 'M16 11c1.66 0 2.99-1.34 2.99-3S17.66 5 16 5c-1.66 0-3 1.34-3 3s1.34 3 3 3zm-8 0c1.66 0 2.99-1.34 2.99-3S9.66 5 8 5C6.34 5 5 6.34 5 8s1.34 3 3 3zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5c0-2.33-4.67-3.5-7-3.5zm8 0c-.29 0-.62.02-.97.05 1.16.84 1.97 1.97 1.97 3.45V19h6v-2.5c0-2.33-4.67-3.5-7-3.5z'
	};
</script>
	...
	<div class="relative max-w-4xl mx-auto px-4 sm:px-6 py-4">
		...
		<div class="mb-4 animate-fade-in-down">
			...
			{#if $assessmentProgress.inferredCount > 0}
				<div class="mt-3 flex justify-center">
					<div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/30 text-xs">
						<svg class="w-4 h-4 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
						</svg>
						<span class="text-amber-400">
							<span class="font-bold">{$assessmentProgress.inferredCount}</span> compétence{$assessmentProgress.inferredCount > 1 ? 's' : ''} inférée{$assessmentProgress.inferredCount > 1 ? 's' : ''}
						</span>
					</div>
				</div>
			{/if}
		...
		{#if showInferenceNotification && lastInferredSkills.length > 0}
			<div class="fixed top-4 right-4 z-50 max-w-sm animate-slide-in-right">
				<div class="card p-4 bg-gradient-to-r from-amber-500/20 to-amber-500/5 border-amber-500/30 shadow-lg">
					<div class="flex items-start gap-3">
						...
						<div class="flex-1 min-w-0">
							<p class="text-sm font-medium text-amber-400">
								{lastInferredSkills.length} compétence{lastInferredSkills.length > 1 ? 's' : ''} inférée{lastInferredSkills.length > 1 ? 's' : ''}
							</p>
							<p class="text-xs text-base-400 mt-1">
								{getInferredSkillNames(lastInferredSkills).slice(0, 2).join(', ')}
								{#if lastInferredSkills.length > 2}
									<span class="text-base-500">+{lastInferredSkills.length - 2} autre{lastInferredSkills.length - 2 > 1 ? 's' : ''}</span>
								{/if}
							</p>
						</div>
						<button
		...
		{#if showGroupTransition && $currentSkillGroup}
			<div class="mb-6 p-4 rounded-xl bg-gradient-to-r from-accent-500/10 to-transparent border border-accent-500/20 animate-fade-in">
				<div class="flex items-center gap-3">
					<div class="w-10 h-10 rounded-lg bg-accent-500/20 flex items-center justify-center">
						<svg class="w-5 h-5 text-accent-400" fill="currentColor" viewBox="0 0 24 24">
							<path d={groupIcons[$currentSkillGroup.groupId || 'devops'] || groupIcons.devops} />
						</svg>
					</div>
					<div>
						<p class="text-sm text-accent-400 font-medium">Nouveau groupe</p>
						<p class="text-base-100 font-bold">{$currentSkillGroup.groupName}</p>
					</div>
				</div>
			</div>
		{/if}
		...
		{#if $currentSkillGroup && !showGroupTransition}
			<div class="mb-2 flex items-center gap-2 text-sm">
				...
				{#if $currentSkill.isCore}
					<span class="px-2 py-0.5 text-xs bg-accent-500/20 text-accent-400 rounded-full">
						Compétence clé
					</span>
				{/if}
			</div>
		...
		<div class="flex justify-between items-center gap-4">
			...
			<!-- Question counter -->
			<div class="flex items-center gap-2 px-4 py-2 rounded-xl bg-base-800/50 border border-base-700/50">
				<span class="font-mono text-sm text-base-400">
					<span class="text-accent-400 font-bold">{$assessmentStore.currentSkillIndex + 1}</span>
		...
		{#if $assessmentProgress.answered > 0 && $assessmentProgress.answered < $assessmentProgress.total}
			<div class="mt-8 text-center animate-fade-in">
				...
				>
					<span class="border-b border-dashed border-base-600 group-hover:border-base-400">
						Terminer maintenant
					</span>
