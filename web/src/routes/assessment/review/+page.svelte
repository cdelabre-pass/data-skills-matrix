<script lang="ts">
	...
	onMount(async () => {
		...
		inferredSkills.sort(
			(a, b) => b.inference.confidence - a.inference.confidence,
		);
	...
	$: reviewedCount = inferredSkills.filter(
		(item) => item.overrideLevel !== null,
	).length;
</script>
...
{#if ready}
	<div class="relative max-w-4xl mx-auto px-4 sm:px-6 py-8">
		...
		<div class="text-center mb-8 animate-fade-in-down">
			<div
				class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-500/10 border border-amber-500/30 mb-4"
			>
				<svg
			...
			<p class="text-base-400 text-lg max-w-2xl mx-auto">
				Basé sur vos réponses, nous avons estimé votre niveau sur certaines
				compétences connexes. Vérifiez et ajustez si nécessaire.
			</p>
		</div>
		...
		{#if inferredSkills.length === 0}
			...
			<div class="card p-8 text-center animate-fade-in">
				...
				<p class="text-base-400 mb-6">
					Aucune compétence n'a été inférée. Vous pouvez consulter vos
					résultats.
				</p>
		...
		{:else}
			...
			<div class="space-y-4">
				{#each inferredSkills as item, index}
					...
					<div
						class="card p-5 transition-all duration-300 animate-fade-in {isReviewed
						...
						<div class="flex flex-col sm:flex-row sm:items-start gap-4">
							...
							<div class="flex-1 min-w-0">
								<div class="flex items-center gap-2 mb-2">
									...
									{#if isReviewed}
										<span
											class="px-2 py-0.5 text-xs bg-green-500/20 text-green-400 rounded-full flex items-center gap-1"
										>
											<svg
												class="w-3 h-3"
												fill="currentColor"
								...
								<p class="text-sm text-base-500 mb-3 line-clamp-2">
								...
								<div class="flex items-center gap-2 text-xs text-base-500">
									...
									<span>
										Basé sur <span class="text-base-400"
											>{getSourceSkillName(item.inference.sourceSkill)}</span
										>
							...
							<div class="flex flex-col items-end gap-2">
								...
								<div class="flex items-center gap-2">
									...
									>
										<span class="text-2xl font-bold text-amber-400"
											>{item.inference.suggestedLevel}</span
										>
										<span class="text-xs text-amber-400/70"
											>{levelLabels[item.inference.suggestedLevel]}</span
										>
									</div>
								</div>
								...
								{#if !isReviewed}
									...
									<div class="flex items-center gap-2 mt-2">
										...
										<div class="relative group">
											...
											<div
												class="absolute right-0 top-full mt-1 py-1 w-48 bg-base-800 border border-base-700 rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10"
											>
												{#each [0, 1, 2, 3, 4, 5, 6] as level}
													...
													>
														<span
															class="w-6 h-6 rounded bg-base-700 flex items-center justify-center font-mono text-xs"
														>
															{level}
														</span>
														<span>{levelLabels[level]}</span>
														{#if level === item.inference.suggestedLevel}
															<span class="text-xs text-amber-500 ml-auto"
																>suggéré</span
															>
														{/if}
													</button>
												...
												>
													<span
														class="w-6 h-6 rounded bg-base-700 flex items-center justify-center"
													>
														<svg
															class="w-3 h-3"
															fill="none"
								...
								{:else}
									...
									<div class="flex items-center gap-2 text-sm text-base-400">
										...
										<span
											class="px-2 py-1 rounded bg-base-700 font-medium text-base-200"
										>
			...
			<div class="mt-8 flex justify-center animate-fade-in">
				...
				>
					<span
						>{allReviewed
							? 'Voir mes résultats'
							: 'Continuer vers les résultats'}</span
					>
					<svg
						class="w-5 h-5"
						fill="none"
